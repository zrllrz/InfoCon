...
@torch.no_grad()
def predict(model, action_hist, state_hist, key_hist, t):
    assert model.model_type in ['s', 's+a', 's+a+k']
    ...
    if not key_hist:  # The first step.
        keys = None
    else:
        keys = torch.stack(key_hist, 1).float().cuda()
    ...
    #
    # if model.use_key_states:
    a_preds, k_preds = model(states, timesteps, actions=actions, key_states=keys)

    return a_preds[:, -1], k_preds[:, -1]  # Only output the last action and key state predictions.


def update(model, action_hist, state_hist, key_hist, actions, states, keys, t):
    ...
    if len(state_hist) == model.block_size // 2:  # The context buffer is full.
        ...
        key_hist = key_hist[1:] + [keys]
        t += 1
    else:
        state_hist.append(states)
        action_hist.append(actions)
        key_hist.append(keys)
    return action_hist, state_hist, key_hist, t
...
if __name__ == "__main__":
    ...
    record_dir = None
    # record_dir = "../replay_policy/" + args.task + '_' + args.model_name + f'_max_step_{args.eval_max_steps}'
    ...
    envs = get_mp_envs(args.task, n_env, start_idx=0, record_dir=record_dir, **env_kwargs)
    ...
    for start_idx in tqdm(range(0, len(eval_ids), n_env)):
        ...
        state_hist, action_hist, key_hist, t = [s], [], [], np.zeros([n_env])

        for step in range(args.eval_max_steps):
            a, ks = predict(model, action_hist, state_hist, key_hist, t)
            a = a.cpu().numpy()
            ks = ks.cpu().numpy()
            ...
            action_hist, state_hist, key_hist, t = update(
                model, action_hist, state_hist, key_hist, a, s, ks, t)
